...
public class WorkReportService {
    ...
    public WorkReport approve(Long id, Long reviewedBy) {
        ...
        // 【改造解耦】审核通过 → 发送 MQ 消息，由工单模块异步累加对应产品明细的已完成数量
        if (workReport.getWorkOrderItemId() != null) {
            java.util.Map<String, Object> msg = new java.util.HashMap<>();
            msg.put("workOrderItemId", workReport.getWorkOrderItemId());
            msg.put("reportedQty", workReport.getReportedQty());
            
            rabbitTemplate.convertAndSend(
                com.zff.springboot_demo.config.RabbitMQConfig.EXCHANGE_MES,
                com.zff.springboot_demo.config.RabbitMQConfig.ROUTING_KEY_WORKREPORT_APPROVED,
                msg
            );
        }
